...
class AppearanceFlowModel(nn.Module):
    ...
    def forward(self, img, flow, mask_prev=None):
    ...
    def _initialize_weights(self, pretrained):
        ...
        # if self.mask:
        #     self.app_blocks[0][0].weight.data[:,3,:,:] = \
        #     torch.from_numpy(copy.deepcopy(
        #         vgg_conv_layers[0].weight.data.numpy().mean(axis=1)))/3
